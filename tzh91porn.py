...
def get_start_urls(page_num=4118, st_num=0, end_num=-1):
    start_urls = ['http://91porn.com/v.php?next=watch&page=%s' % i for i in range(1, page_num+1)]
    print(start_urls)
    return start_urls[st_num:end_num]
...
def parse_main_page(url):
	# r = requests.get(url, headers=setHeader(), cookies=cookies, proxies=proxies)
	r = getContent(url)
	# print(r.status_code)
...
if __name__ == '__main__':
	start_urls = get_start_urls(page_num=10, st_num=5, end_num=10)
	for url in start_urls:
		parse_main_page(url=url)
